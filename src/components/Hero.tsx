...
export function Hero({ locale = "en" }: { locale?: Locale }) {
  ...
  const { hero, identity, ui } = getContent(locale);
  const t = ui.hero;

  return (
    <section id="top" className="relative isolate overflow-hidden px-6 pb-12 pt-28 sm:px-8 sm:pb-16 sm:pt-32 lg:min-h-[calc(100svh-4rem)] lg:px-12 lg:pb-12 lg:ps-24 lg:pt-32">
      ...
      <div className="mx-auto w-full max-w-[1320px]">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4 border-b border-paper/15 pb-4 sm:mb-10">
          ...
          <span className="font-mono-ui text-[0.62rem] tracking-[0.12em] text-paper/40 uppercase">{t.location} <span className="mx-2 text-wire">/</span> {t.locationTag}</span>
        </div>

        <div className="grid items-center gap-12 lg:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)] lg:gap-8 xl:gap-16">
          <div className="relative z-10">
            <motion.p initial={reduce ? undefined : { opacity: 0, y: 10 }} animate={reduce ? undefined : { opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="section-label eyebrow text-wire">{t.eyebrow}</motion.p>
            ...
            <motion.h1 initial="hidden" animate="show" className="mt-5 font-display text-[clamp(2.8rem,12vw,4.5rem)] font-semibold uppercase leading-[0.84] tracking-[-0.05em] text-paper lg:text-[clamp(4rem,7vw,7.2rem)] lg:leading-[0.8]">
              <motion.span variants={{ hidden: { opacity: 0, y: 28 }, show: { opacity: 1, y: 0, transition: { duration: 0.65, ease: [0.16, 1, 0.3, 1] } } }} className="block">{hero.nameLines[0]}</motion.span>
              <motion.span variants={{ hidden: { opacity: 0, y: 28 }, show: { opacity: 1, y: 0, transition: { duration: 0.65, delay: reduce ? 0 : 0.1, ease: [0.16, 1, 0.3, 1] } } }} className="block text-signal">{hero.nameLines[1]}<span className="text-wire">.</span></motion.span>
            </motion.h1>
            ...
            <motion.div initial={reduce ? undefined : { opacity: 0, y: 12 }} animate={reduce ? undefined : { opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: reduce ? 0 : 0.4 }} className="mt-7 flex flex-wrap items-center gap-x-7 gap-y-3">
              <motion.a whileHover={reduce ? undefined : { y: -2 }} whileTap={{ scale: 0.98 }} href={hero.ctaPrimary.href} className="inline-flex min-h-12 items-center gap-5 rounded-xl border border-signal bg-signal px-5 font-mono-ui text-xs tracking-[0.08em] text-console shadow-[0_8px_24px_rgba(201,162,39,0.12)] transition-colors duration-200 hover:bg-transparent hover:text-signal">
                {hero.ctaPrimary.label}<span aria-hidden="true" className="flip-rtl">↗</span>
              </motion.a>
              <motion.a whileHover={reduce ? undefined : { x: locale === "ar" ? -4 : 4 }} href={hero.ctaSecondary.href} className="inline-flex min-h-12 items-center gap-3 font-mono-ui text-xs tracking-[0.08em] text-paper/75 uppercase transition-colors hover:text-paper">
                {hero.ctaSecondary.label}<span className="flip-rtl text-signal" aria-hidden="true">→</span>
              </motion.a>
            </motion.div>
          </div>
          ...
          <motion.figure initial={reduce ? undefined : { opacity: 0, scale: 0.96, y: 14 }} animate={reduce ? undefined : { opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.8, delay: reduce ? 0 : 0.2, ease: [0.16, 1, 0.3, 1] }} className="relative mx-auto w-full max-w-[600px] lg:max-w-none" aria-labelledby="network-caption">
            <div className="relative overflow-visible py-2 sm:py-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="eyebrow text-paper/65">{t.diagramTitle}</p>
                  <p className="mt-1 font-mono-ui text-[0.62rem] tracking-[0.08em] text-paper/35 uppercase">{t.diagramSub}</p>
                </div>
                <span className="inline-flex items-center gap-2 font-mono-ui text-[0.6rem] tracking-[0.08em] text-[#a4d3a7] uppercase"><span className="h-1.5 w-1.5 rounded-full bg-[#a4d3a7]" aria-hidden="true" /> {t.live}</span>
              </div>

              <div className="network-stage relative mx-auto mt-3 aspect-[1.18] w-full max-w-[560px]" role="img" aria-label={t.diagramAria}>
                ...
                <svg viewBox="0 0 500 420" style={{ direction: "ltr" }} className="absolute inset-0 h-full w-full" aria-hidden="true">
                  <defs>
                ...
                <span className="absolute bottom-2 start-2 font-mono-ui text-[0.55rem] tracking-[0.12em] text-paper/35 uppercase">{t.nodes}</span>
              </div>

              <div className="grid grid-cols-3 border-t border-paper/15 pt-4">
                {t.stats.map((stat, index) => (
                  <div key={stat.label} className={index === 0 ? "pe-3 sm:pe-5" : index === 1 ? "border-s border-paper/15 px-3 sm:px-5" : "border-s border-paper/15 ps-3 sm:ps-5"}>
                    <p className="font-display text-3xl font-semibold leading-none text-paper sm:text-4xl">{stat.value}{stat.suffix && <span className={index === 0 ? "text-signal" : "text-wire"}>{stat.suffix}</span>}</p>
                    <p className="mt-2 font-mono-ui text-[0.58rem] tracking-[0.1em] text-paper/55 uppercase sm:text-[0.65rem]">{stat.label}</p>
                  </div>
                ))}
              </div>
              <figcaption id="network-caption" className="sr-only">{t.figcaption}</figcaption>
            </div>
          </motion.figure>
        </div>
